...
public class ProtocolloDAOjdbc implements ProtocolloDAO {
	...
	public ReportProtocolloView getProtocolloView(Connection connection, int id) throws DataException {
		ReportProtocolloView protocollo = new ReportProtocolloView();
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		try {
			if (connection == null) {
				logger.warn("getProtocolloView() - Invalid Connection :" + connection);
	...
	public ProtocolloVO getProtocolloById(Connection connection, int id) throws DataException {
		ProtocolloVO protocollo = new ProtocolloVO();
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		try {
			if (connection == null) {
				logger.warn("getProtocolloById() - Invalid Connection :" + connection);
				throw new DataException("Connessione alla base dati non valida.");
			}
			pstmt = connection.prepareStatement(SELECT_PROTOCOLLO_BY_ID);
			pstmt.setInt(1, id);
	...
	private void archiviaVersione(Connection connection, int protocolloId, int versione) throws DataException {
		String[] tables = { "protocolli", "protocollo_allacci", "protocollo_allegati", "protocollo_annotazioni",
				"protocollo_assegnatari", "protocollo_destinatari", "protocollo_procedimenti" };
		...
		try {
			...
			for (int i = 0; i < tables.length; i++) {
				String sql = "INSERT INTO storia_" + tables[i] + " SELECT * FROM " + tables[i]
						+ " WHERE protocollo_id = ?";
				// logger.debug(sql);
	 ...
	 * 
	 * @see
	 * it.finsiel.siged.mvc.integration.ProtocolloDAO#newProtocollo(java.sql.
	 * Connection, it.finsiel.siged.mvc.vo.protocollo.ProtocolloVO)
	 * 
	 */
	public ProtocolloVO newProtocollo(Connection connection, ProtocolloVO protocollo) throws DataException {
	 ...
	 * 
	 */
	public ProtocolloVO newStoriaProtocollo(Connection connection, ProtocolloVO protocollo) throws DataException {
		ProtocolloVO newProtocollo = new ProtocolloVO();
		newProtocollo.setReturnValue(ReturnValues.UNKNOWN);
		PreparedStatement pstmt = null;
		try {
	...
	public ProtocolloVO aggiornaProtocollo(Connection connection, ProtocolloVO protocollo) throws DataException {
			...
			if (p.getVersione() == protocollo.getVersione()) {
				...
				if (result == 1) {
					...
					if (protocollo.getReturnValue() == ReturnValues.FOUND) {
						...
						logger.warn("Errore nella lettura del ProtocolloVO dopo l'aggiornamento! Id:" + protocolloId);
	 ...
	 * 
	 * @see
	 * it.finsiel.siged.mvc.integration.ProtocolloDAO#getMaxNumProtocollo(java.
	 * sql.Connection, int, int)
	 */
	public int getMaxNumProtocollo(Connection connection, int anno, int registro) throws DataException {
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		try {
			if (connection == null) {
			...
			if (rs.next())
				...
				return 1;
			// TODO: migliorare la presa in carico del primo numero di
			// protocollo
	...
	private int getNextNumber() {
		...
		if (next != 1) {
			...
			File properties = new File(path); // TODO controllare bene il path
												// per il file properties
	 ...
	 * 
	 * @see
	 * it.finsiel.siged.mvc.integration.ProtocolloDAO#getProtocollo_By_Numero(
	 * int, int, int)
	 */
	public ProtocolloVO getProtocolloByNumero(int anno, int registro, int numProtocollo) throws DataException {
		ProtocolloVO protocollo = new ProtocolloVO();
		Connection connection = null;
		try {
			connection = jdbcMan.getConnection();
			protocollo = getProtocolloByNumero(connection, anno, registro, numProtocollo);
	...
	public ProtocolloVO getProtocolloByNumero(Connection connection, int anno, int registro, int numProtocollo)
		...
		try {
			if (connection == null) {
				logger.warn("getProtocolloByNumero (Connection connection, int anno, int registro,"
						+ "int numProtocollo) - Invalid Connection :" + connection);
				throw new DataException("Connessione alla base dati non valida.");
			}
	...
	public void aggiornaDocumentoPrincipaleId(Connection connection, int protocolloId, int documentoId)
			throws DataException {
		PreparedStatement pstmt = null;
		try {
			if (connection == null) {
				logger.warn("aggiornaDocumentoPrincipaleId() - Invalid Connection :" + connection);
				throw new DataException("Connessione alla base dati non valida.");
			}
	...
	public void salvaAllegato(Connection connection, int protocollo_allegati_id, int protocolloId, int documentoId,
			int versione) throws DataException {
		PreparedStatement pstmt = null;

		try {
			if (connection == null) {
				logger.warn("salvaAllegato() - Invalid Connection :" + connection);
				throw new DataException("Connessione alla base dati non valida.");
			}
	 ...
	 * 
	 * @see
	 * it.finsiel.siged.mvc.integration.ProtocolloDAO#insertAllacci(java.sql.
	 * Connection, int, java.util.Collection)
	 */

	public void salvaAllaccio(Connection connection, AllaccioVO allaccio) throws DataException {
		PreparedStatement pstmt = null;
		try {
			if (connection == null) {
				logger.warn("salvaAllaccio() - Invalid Connection :" + connection);
				throw new DataException("Connessione alla base dati non valida.");
			}
			if (allaccio != null) {
	 ...
	 * 
	 * @see
	 * it.finsiel.siged.mvc.integration.ProtocolloDAO#getProtocolliAllacciabili(
	 * int, int, int)
	 */

	public Collection getProtocolliAllacciabili(Utente utente, int numeroProtocolloDa, int numeroProtocolloA,
			int annoProtocollo, int protocolloId) throws DataException {
		...
		if (!uff.getValueObject().getTipo().equals(UfficioVO.UFFICIO_CENTRALE)) {
			...
			strQuery.append(" AND (EXISTS (SELECT * FROM ").append("protocollo_assegnatari ass ")
					.append("WHERE ass.protocollo_id=p.protocollo_id ").append("AND ass.ufficio_assegnatario_id IN (")
					.append(ufficiUtenti).append(")) OR").append(" p.ufficio_protocollatore_id IN (")
					.append(ufficiUtenti).append(") OR").append(" p.ufficio_mittente_id IN (").append(ufficiUtenti)
					.append("))");
		}
		...
		try {
			...
			while (rs.next()) {
				...
				allaccio.setDataProtocollo(DateUtil.formattaData(rs.getDate("data_registrazione").getTime()));
				...
				if (rs.getString("FLAG_TIPO_MITTENTE").equals("F")) {
					allaccio.setMittente(rs.getString("DESC_COGNOME_MITTENTE") + " "
							+ StringUtil.getStringa(rs.getString("DESC_NOME_MITTENTE")));
				} else {
					allaccio.setMittente(rs.getString("DESC_DENOMINAZIONE_MITTENTE"));
				}
	...
	public Map getAllegatiProtocollo(int protocolloId) throws DataException {
		...
		try {
			...
			while (rs.next()) {
				...
				DocumentoVO doc = documentoDelegate.getDocumento(connection, id);
	...
	public Collection getAllacciProtocollo(int protocolloId) throws DataException {
		...
		try {
			...
			throw new DataException(e.getMessage() + " ProtocolloId :" + protocolloId);
		} finally {
			jdbcMan.close(connection);
		}
		return allacci;
	...
	public Collection getAllacciProtocollo(Connection connection, int protocolloId) throws DataException {
		ArrayList allacci = new ArrayList();
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		try {
			...
			while (rs.next()) {
				...
				allaccio.setProtocolloAllacciatoId(rs.getInt("protocollo_allacciato_id"));
				allaccio.setPrincipale(rs.getBoolean("flag_principale"));
				allaccio.setAllaccioDescrizione(rs.getInt("nume_protocollo") + "/" + rs.getInt("anno_registrazione")
	...
	public AssegnatarioVO getAssegnatarioPerCompetenza(int protocolloId) throws DataException {
		AssegnatarioVO assegnatario = new AssegnatarioVO();
		Connection connection = null;
		try {
			connection = jdbcMan.getConnection();
			assegnatario = getAssegnatarioPerCompetenza(connection, protocolloId);
		} catch (Exception e) {
			throw new DataException(e.getMessage() + " ProtocolloId :" + protocolloId);
		} finally {
			jdbcMan.close(connection);
	...
	public AssegnatarioVO getAssegnatarioPerCompetenza(Connection connection, int protocolloId) throws DataException {
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		AssegnatarioVO assegnatario = new AssegnatarioVO();
		try {
			...
			if (rs.next()) {
				...
				assegnatario.setDataAssegnazione(rs.getDate("data_assegnazione"));
				assegnatario.setDataOperazione(rs.getDate("data_operazione"));
				assegnatario.setUfficioAssegnanteId(rs.getInt("ufficio_assegnante_id"));
				assegnatario.setUfficioAssegnatarioId(rs.getInt("ufficio_assegnatario_id"));
			...
			throw new DataException("Cannot load getAssegnatarioPerCompetenzaProtocollo");
		} finally {
			jdbcMan.close(rs);
			jdbcMan.close(pstmt);
		}
	...
	public Collection getAssegnatariProtocollo(int protocolloId) throws DataException {
		...
		try {
			...
			while (rs.next()) {
				...
				assegnatario.setCompetente(rs.getBoolean("flag_competente"));
				assegnatario.setUtenteAssegnanteId(rs.getInt("utente_assegnante_id"));
				assegnatario.setStatoAssegnazione(rs.getString("stat_assegnazione").charAt(0));
				assegnatario.setMsgAssegnatarioCompetente(rs.getString("messaggio"));
	...
	public void salvaAssegnatario(Connection connection, AssegnatarioVO assegnatario, int versione)
			throws DataException {
		PreparedStatement pstmt = null;
		int recIns = 0;
		...
		try {
			if (connection == null) {
				logger.warn("salvaAssegnatario() - Invalid Connection :" + connection);
				throw new DataException("Connessione alla base dati non valida.");
			}
			if (assegnatario != null) {
				pstmt = connection.prepareStatement(INSERT_PROTOCOLLO_ASSEGNATARI);
				...
				pstmt.setString(11, assegnatario.getMsgAssegnatarioCompetente());
	...
	public int presaIncarico(Connection connection, ProtocolloVO protocolloVO, String tipoAzione, Utente utente)
			throws DataException {
		...
		try {
			if (connection == null) {
				logger.warn("presaIncarico() - Invalid Connection :" + connection);
				throw new DataException("Connessione alla base dati non valida.");
			}
			/*
			...
			if ("R".equals(tipoAzione)) {
				sqlUpdateAssegnatarioCompetenza.append(", utente_assegnatario_id=utente_assegnante_id, "
						+ "ufficio_assegnatario_id=ufficio_assegnante_id," + "ufficio_assegnante_id="
						+ utente.getUfficioInUso() + "," + "utente_assegnante_id="
						+ utente.getValueObject().getId().intValue());
			} else {
				sqlUpdateAssegnatarioCompetenza.append(", utente_assegnatario_id=?");
			}
			sqlUpdateAssegnatarioCompetenza.append(" WHERE protocollo_id=? AND flag_competente=1 AND versione=?");

			pstmt = connection.prepareStatement(sqlUpdateAssegnatarioCompetenza.toString());
			// logger.debug(" presaIncarico: " + protocolloVO.getId());
			int indice = 1;
			pstmt.setString(indice++, tipoAzione);
			if (!"R".equals(tipoAzione)) {
				pstmt.setInt(indice++, utente.getValueObject().getId().intValue());
			}
	...
	public Map getProtocolliAssegnati(Utente utente, int annoProtocolloDa, int annoProtocolloA, int numeroProtocolloDa,
			int numeroProtocolloA, java.util.Date dataDa, java.util.Date dataA, String statoProtocollo,
			String statoScarico, String tipoUtenteUfficio) throws DataException {
		SortedMap<Integer, ReportProtocolloView> protocolli = new TreeMap<Integer, ReportProtocolloView>(
		...
		String strQuery = "SELECT p.*, a.ufficio_assegnante_id, a.ufficio_assegnatario_id, "
				...
				+ " FROM PROTOCOLLI p, PROTOCOLLO_ASSEGNATARI a " + " WHERE registro_id=? AND stato_protocollo=? "
				+ " AND p.protocollo_id =a.protocollo_id  AND";
		if (annoProtocolloDa > 0) {
			strQuery = strQuery + " ANNO_REGISTRAZIONE>=" + annoProtocolloDa + " AND";
		}
		if (annoProtocolloA > 0) {
			strQuery = strQuery + " ANNO_REGISTRAZIONE<=" + annoProtocolloA + " AND";
		}
		if (numeroProtocolloDa > 0) {
			strQuery = strQuery + " NUME_PROTOCOLLO>=" + numeroProtocolloDa + " AND";
		}
		if (dataDa != null) {
		...
		if (numeroProtocolloA > 0) {
			strQuery = strQuery + " NUME_PROTOCOLLO<=" + numeroProtocolloA + " AND";
		}
		if ("S".equals(statoScarico) && "T".equals(tipoUtenteUfficio)) {
			strQuery = strQuery + " utente_assegnatario_id=? AND flag_competente=1 AND STAT_ASSEGNAZIONE=?";

		} else if (("S".equals(statoScarico) && "U".equals(tipoUtenteUfficio)) || "A".equals(statoScarico)
				|| "R".equals(statoScarico)) {
			strQuery = strQuery + " ufficio_assegnatario_id IN (" + utente.getUfficiIds() + ") ";
			strQuery = strQuery + " AND utente_assegnatario_id IS NULL";
			strQuery = strQuery + " AND flag_competente=1 AND STAT_ASSEGNAZIONE=?";
		...
		try {
			...
			if (dataA != null) {
				pstmt.setTimestamp(currPstmt++, new Timestamp(dataA.getTime() + Constants.GIORNO_MILLISECONDS - 1));
			}
			if (("S".equals(statoScarico) && "T".equals(tipoUtenteUfficio)) || "N".equals(statoScarico)) {
				pstmt.setInt(currPstmt++, utente.getValueObject().getId().intValue());
			}
			...
			while (rs.next()) {
				...
				protocollo.setDataProtocollo(DateUtil.formattaData(rs.getDate("DATA_REGISTRAZIONE").getTime()));
				...
				if (rs.getInt("FLAG_RISERVATO") == 1) {
					...
					protocollo.setMittente(StringUtil.getStringa(rs.getString("DESC_COGNOME_MITTENTE")) + " "
							+ StringUtil.getStringa(rs.getString("DESC_NOME_MITTENTE"))
							+ StringUtil.getStringa(rs.getString("DESC_DENOMINAZIONE_MITTENTE")));
				}
				protocollo.setUfficio(rs.getString("ufficio_assegnante_id"));
				protocollo.setUtenteAssegnante(rs.getString("utente_assegnante_id"));
				protocollo.setUfficioAssegnatario(rs.getString("ufficio_assegnatario_id"));
				protocollo.setUtenteAssegnatario(rs.getString("utente_assegnatario_id"));
				protocollo.setPdf(rs.getInt("DOCUMENTO_ID") > 0 ? true : false);
				protocollo.setDocumentoId(rs.getInt("DOCUMENTO_ID"));
				protocollo.setMessaggio(StringUtil.getStringa(rs.getString("messaggio")));
				if (rs.getDate("data_scarico") != null)
					protocollo.setDataScarico(DateUtil.formattaData(rs.getDate("data_scarico").getTime()));
				protocolli.put(new Integer(protocollo.getProtocolloId()), protocollo);
			}
		} catch (Exception e) {
	...
	@SuppressWarnings("rawtypes")
	public Map getProtocolliRespinti(Utente utente, int annoProtocolloDa, int annoProtocolloA, int numeroProtocolloDa,
			int numeroProtocolloA, java.util.Date dataDa, java.util.Date dataA) throws DataException {
		SortedMap<Integer, ReportProtocolloView> protocolli = new TreeMap<Integer, ReportProtocolloView>(
				new Comparator<Integer>() {
					public int compare(Integer o1, Integer o2) {
						return o2.intValue() - o1.intValue();
		...
		String strQuery = "SELECT P.*, ufficio_assegnatario_id, utente_assegnatario_id, "
				...
				+ " AND stat_assegnazione='R' " + " AND utente_assegnatario_id ="
				+ utente.getValueObject().getId().intValue();
		if (annoProtocolloDa > 0) {
			strQuery = strQuery + " AND ANNO_REGISTRAZIONE>=" + annoProtocolloDa + " ";
		}
		if (annoProtocolloA > 0) {
			strQuery = strQuery + " AND ANNO_REGISTRAZIONE<=" + annoProtocolloA;
		}
		if (numeroProtocolloDa > 0) {
			strQuery = strQuery + " AND  NUME_PROTOCOLLO>=" + numeroProtocolloDa;
		}
		if (dataDa != null) {
		...
		try {
			...
			while (rs.next()) {
				...
				protocollo.setUfficioAssegnatario("" + rs.getInt("ufficio_assegnante_id"));
				protocollo.setUtenteAssegnatario("" + rs.getInt("utente_assegnante_id"));
	...
	public int updateScarico(Connection connection, ProtocolloVO protocolloVO, String flagScarico, Utente utente)
			throws DataException {
		PreparedStatement pstmt = null;
		int fglagStatus = ReturnValues.SAVED;
		...
		try {
			if (connection == null) {
				logger.warn("updateScarico() - Invalid Connection :" + connection);
				throw new DataException("Connessione alla base dati non valida.");
			}
			archiviaVersione(connection, protocolloVO.getId().intValue(), protocolloVO.getVersione() + 1);
			...
			if (n == 0) {
				...
				if ("R".equals(flagScarico) || "A".equals(flagScarico)) {
					...
					sqlUpdate = "UPDATE PROTOCOLLO_ASSEGNATARI set" + " ufficio_assegnatario_id=ufficio_assegnante_id,"
							+ " utente_assegnatario_id=utente_assegnante_id," + " stat_assegnazione='R',"
							+ " ufficio_assegnante_id=" + utente.getUfficioInUso() + "," + " utente_assegnante_id="
							+ utente.getValueObject().getId().intValue()
							+ " WHERE protocollo_id=? AND flag_competente=1";
	...
	public SortedMap cercaProtocolli(Utente utente, Ufficio uff, HashMap sqlDB) throws DataException {
			...
			if (sqlDB != null) {
				for (Iterator it = sqlDB.entrySet().iterator(); it.hasNext();) {
					...
					if (value instanceof Integer) {
						...
						pstmt.setTimestamp(indiceQuery, new java.sql.Timestamp(d.getTime()));
					} else if (value instanceof Boolean) {
						pstmt.setBoolean(indiceQuery, ((Boolean) value).booleanValue());
				...
				if (!rs.getBoolean("flag_riservato")) {
					...
					if ("I".equals(protocollo.getTipoProtocollo())) {
						...
						if (rs.getString("cognome_assegnatario") != null) {
							dest_ass.append(" / ").append(rs.getString("cognome_assegnatario"));
							if (rs.getString("nome_assegnatario") != null) {
								dest_ass.append(' ').append(rs.getString("nome_assegnatario"));
							}
						}
						if (rs.getBoolean("flag_competente")) {
							dest_ass.append("</em>");
	...
	public Collection getMittenti(String mittente) throws DataException {
		...
		try {
			...
			while (rs.next()) {
				...
				mittenteView.setNome(StringUtil.getStringa(rs.getString("DESC_NOME_MITTENTE")));
				mittenteView.setDenominazione(rs.getString("DESC_DENOMINAZIONE_MITTENTE"));
	...
	public Map getDestinatariProtocollo(int protocolloId) throws DataException {
		...
		try {
			...
			while (rs.next()) {
				...
				destinatarioVO.setFlagTipoDestinatario(rs.getString("flag_tipo_destinatario"));
				...
				destinatarioVO.setFlagConoscenza("1".equals(rs.getString("flag_conoscenza")));
				destinatarioVO.setProtocolloId(rs.getInt("protocollo_id"));
				destinatarioVO.setDataEffettivaSpedizione(rs.getDate("data_effettiva_spedizione"));
				destinatarioVO.setVersione(rs.getInt("versione"));
				destinatarioVO.setMezzoSpedizioneId(rs.getInt("mezzo_spedizione_id"));
				destinatarioVO.setMezzoDesc(rs.getString("desc_spedizione"));
				destinatarioVO.setCodicePostale(rs.getString("codice_postale"));
				destinatari.put(String.valueOf(destinatarioVO.getId()), destinatarioVO);
			}
		} catch (Exception e) {
	...
	public int annullaProtocollo(Connection connection, ProtocolloVO protocolloVO, Utente utente) throws DataException {
		PreparedStatement pstmt = null;
		int fglagStatus = ReturnValues.SAVED;
		try {
			Date toDay = new Date(System.currentTimeMillis());
			if (connection == null) {
				logger.warn("annullaProtocollo() - Invalid Connection :" + connection);
				throw new DataException("Connessione alla base dati non valida.");
			}
			archiviaVersione(connection, protocolloVO.getId().intValue(), protocolloVO.getVersione() + 1);
	...
	public int salvaSegnatura(Connection connection, SegnaturaVO segnaturaVO) throws DataException {
		PreparedStatement pstmt = null;
		int fglagStatus = ReturnValues.SAVED;
		try {
			if (connection == null) {
				logger.warn("salvaSignature() - Invalid Connection :" + connection);
				throw new DataException("Connessione alla base dati non valida.");
			}
	...
	public void eliminaDestinatariProtocollo(Connection connection, int protocolloId) throws DataException {
		PreparedStatement pstmt = null;
		try {
	...
	public void salvaDestinatario(Connection connection, DestinatarioVO destinatario, int versione)
			throws DataException {
		PreparedStatement pstmt = null;
		int recIns = 0;
		...
		try {
			if (connection == null) {
				logger.warn("salvaDestinatario() - Invalid Connection :" + connection);
				throw new DataException("Connessione alla base dati non valida.");
			}

			if (destinatario != null) {
				pstmt = connection.prepareStatement(INSERT_PROTOCOLLO_DESTINATARI);
				java.sql.Date now = new java.sql.Date((new java.util.Date()).getTime());
				...
				if (destinatario.getDataSpedizione() != null) {
					pstmt.setDate(7, new java.sql.Date(destinatario.getDataSpedizione().getTime()));
				} else {
	...
	public Collection getProtocolliByProtMittente(Utente utente, String protMittente) throws DataException {
		...
		try {
			...
			while (rs.next()) {
				...
				protocolloView.setAnnoProtocollo(rs.getInt("ANNO_REGISTRAZIONE"));
				protocolloView.setNumeroProtocollo(rs.getInt("NUME_PROTOCOLLO"));
				protocolloView.setDataProtocollo(DateUtil.formattaData(rs.getDate("data_registrazione").getTime()));
				protocolloView.setTipoProtocollo(rs.getString("FLAG_TIPO"));
				protocolloView.setOggetto(rs.getString("text_oggetto"));
				if (rs.getString("flag_tipo_mittente").equals("F")) {
					protocolloView.setMittente(rs.getString("desc_cognome_mittente") + " "
							+ StringUtil.getStringa(rs.getString("desc_nome_mittente")));
				} else {
					protocolloView.setMittente(rs.getString("DESC_DENOMINAZIONE_MITTENTE"));
				}
	...
	public Collection getMezziSpedizione(Connection connection, String idMezzi) throws DataException {
	...
	public Map getMezziSpedizioneByProteDest(int protocolloId, int destinatarioId) throws DataException {
	...
	public boolean esisteAllaccio(Connection connection, int protocolloId, int protocolloAllacciatoId)
			throws DataException {
		PreparedStatement pstmt = null;
		ResultSet rs = null;
	...
	public void eliminaAllacciProtocollo(Connection connection, int protocolloId) throws DataException {
		PreparedStatement pstmt = null;
	...
	public void eliminaAllaccioProtocollo(Connection connection, int protocolloId, int protocolloAllacciatoId)
			throws DataException {
		PreparedStatement pstmt = null;
	...
	public void eliminaAssegnatariProtocollo(Connection connection, int protocolloId) throws DataException {
		PreparedStatement pstmt = null;
	...
	public int riassegnaProtocollo(ProtocolloIngresso protocollo, Utente utente) throws DataException {
		Connection connection = null;
		...
		try {
			...
			if (assegnatari != null) {
				...
				for (Iterator i = assegnatari.iterator(); i.hasNext();) {
					...
					assegnatario.setId(IdentificativiDelegate.getInstance().getNextId(connection,
	...
	public Collection getProtocolliToExport(Connection connection, int registroId) throws DataException {
		...
		try {
			...
			while (rs.next()) {
				...
				protocollo.setDataRegistrazione(rs.getDate("data_registrazione"));
	...
	public void updateRegistroEmergenza(Connection connection) throws DataException {
		PreparedStatement pstmt = null;
		try {
			if (connection == null) {
				logger.warn("updateRegistroEmergenza() - Invalid Connection :" + connection);
				throw new DataException("Connessione alla base dati non valida.");
			}
			pstmt = connection.prepareStatement(
					"UPDATE PROTOCOLLI SET num_prot_emergenza = nume_protocollo WHERE num_prot_emergenza=0 OR num_prot_emergenza IS NULL");
			pstmt.executeUpdate();
		} catch (Exception e) {
	...
	public void updateMsgAssegnatarioCompetenteByIdProtocollo(Connection connection, String msgAssegnatarioCompetente,
			int protocolloId) throws DataException {
		PreparedStatement pstmt = null;
		try {
			if (connection == null) {
				logger.warn("updateMsgAssegnatarioCompetenteByIdProtocollo() - Invalid Connection :" + connection);
				throw new DataException("Connessione alla base dati non valida.");
			}
	...
	public Collection getProcedimentiProtocollo(int protocolloId) throws DataException {
		...
		try {
			...
			while (rs.next()) {
				...
				procedimento.setNumeroProcedimento(
						rs.getString("anno") + StringUtil.formattaNumeroProcedimento(rs.getString("numero"), 7));
	...
	public void inserisciProcedimenti(Connection connection, ArrayList ids) throws DataException {
	...
	public void inserisciProcedimenti(Connection connection, ProtocolloProcedimentoVO protocolloProcedimentiVO)
			throws DataException {
		...
		try {
			...
			if (protocolloProcedimentiVO.getRowCreatedTime() != null) {
				pstmt.setDate(3, new Date(protocolloProcedimentiVO.getRowCreatedTime().getTime()));
			} else {
				pstmt.setDate(3, null);
			}
			if (protocolloProcedimentiVO.getRowUpdatedTime() != null) {
				pstmt.setDate(6, new Date(protocolloProcedimentiVO.getRowUpdatedTime().getTime()));
			} else {
	...
	public void cancellaProcedimenti(Connection connection, int protocolloId) throws DataException {
		PreparedStatement pstmt = null;
		try {
			if (connection == null) {
				logger.warn("Invalid Connection :" + connection);
	...
	public boolean isUtenteAbilitatoView(Utente utente, Ufficio uff, int protocolloId) throws DataException {
		...
		try {
			...
			if (!uff.getValueObject().getTipo().equals(UfficioVO.UFFICIO_CENTRALE)) {
				...
				strQuery.append(" AND (EXISTS (SELECT * FROM ").append("protocollo_assegnatari ass ")
						...
						.append("AND ass.ufficio_assegnatario_id IN (").append(ufficiUtenti).append(")) OR")
						.append(" p.ufficio_protocollatore_id IN (").append(ufficiUtenti).append(") OR")
						.append(" p.ufficio_mittente_id IN (").append(ufficiUtenti).append("))");
			}
	...
	private final static String INSERT_PROTOCOLLO_DESTINATARI = "INSERT INTO protocollo_destinatari"
			...
			+ ", titolo_id, mezzo_spedizione,note,codice_postale " + ")" + " VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?"
			+ ", ?" + ", ?,?,?)";
	...
	public final static String ANNULLA_PROTOCOLLO = "UPDATE PROTOCOLLI SET stato_protocollo=?, data_annullamento=?, "
			+ "text_nota_annullamento=?, text_provvedimento_annullament=?" + " WHERE protocollo_id=? AND versione=?";
	...
	private final static String UPDATE_PROTOCOLLO = "UPDATE protocolli SET"
			...
			+ " num_prot_emergenza = ?, motivazione_modifica = ?" + " WHERE protocollo_id = ? AND versione = ?";
	...
	public final static String SELECT_LISTA_PROTOCOLLI_STORIA = "SELECT "
			...
			+ "p.desc_denominazione_mittente,ut.nome as nome_assegnatario," + "uf.descrizione as ufficio_assegnatario,"
			...
			+ " LEFT JOIN protocollo_destinatari d ON d.protocollo_id=p.protocollo_id " + "WHERE p.protocollo_id=?";
	...
	public final static String SELECT_LISTA_PROTOCOLLI = "SELECT "
			...
			+ " LEFT JOIN uffici uf ON a.ufficio_assegnatario_id=uf.ufficio_id"
			+ " LEFT JOIN utenti ut ON a.utente_assegnatario_id=ut.utente_id"
			+ " LEFT JOIN protocollo_destinatari d ON d.protocollo_id=p.protocollo_id " + "WHERE p.registro_id=?";

	public final static String SELECT_ALLACCI = "SELECT DISTINCT "
			+ "p.protocollo_id,p.anno_registrazione,p.nume_protocollo,"
			+ "p.flag_tipo,p.flag_riservato,p.text_oggetto,p.data_registrazione,"
			+ "p.flag_tipo_mittente,p.desc_cognome_mittente,p.desc_nome_mittente," + "p.desc_denominazione_mittente "
			+ "FROM protocolli p" + " LEFT JOIN protocollo_assegnatari a ON a.protocollo_id=p.protocollo_id"
			+ " LEFT JOIN uffici uf ON a.ufficio_assegnatario_id=uf.ufficio_id"
			+ " LEFT JOIN utenti ut ON a.utente_assegnatario_id=ut.utente_id"
